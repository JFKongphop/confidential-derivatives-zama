...
async function doFulfillTrigger(
  ...
  const reqTx = await futures
    .connect(caller)
    .checkTrigger(user.address, positionId);
  const receipt = await reqTx.wait();
  // Extract requestId from TriggerCheckRequested event
  const event = receipt!.logs.find(
    (l) =>
      futures.interface.parseLog(l as any)?.name === "TriggerCheckRequested",
  );
...
describe("Stop-Loss / Take-Profit & PnL History", function () {
  ...
  describe("setStopLoss", function () {
    it("stores an encrypted SL trigger for an open position", async function () {
      ...
      const { handle, inputProof } = await encryptTriggerPrice(
        c.futures,
        signers.alice,
        PRICE_1800,
      );
      await expect(
        c.futures.connect(signers.alice).setStopLoss(0n, handle, inputProof),
      )
  ...
  describe("setTakeProfit", function () {
    it("stores an encrypted TP trigger for an open position", async function () {
      ...
      await expect(
        c.futures.connect(signers.alice).setTakeProfit(0n, handle, inputProof),
      )
  ...
  describe("checkTrigger", function () {
    it("creates a pendingTriggers entry when a trigger is set", async function () {
      ...
      await c.futures
        ...
        .setStopLoss(0n, handle, inputProof);

      await expect(
        c.futures
          .connect(signers.keeper)
          .checkTrigger(signers.alice.address, 0n),
      )
  ...
  describe("fulfillTrigger — SL long", function () {
    it("closes position when SL fires (price drops below SL)", async function () {
      ...
      const { tx } = await doFulfillTrigger(
        c.futures,
        signers.alice,
        0n,
        signers.keeper,
  ...
  describe("fulfillTrigger — TP long", function () {
    it("closes position when TP fires (price rises above TP)", async function () {
      ...
      await c.futures
        .connect(signers.alice)
        .setTakeProfit(0n, handle, inputProof);
      const { tx } = await doFulfillTrigger(
        c.futures,
        signers.alice,
        0n,
  ...
  describe("Encrypted PnL History", function () {
    it("getMyRealizedPnL returns an encrypted handle after fulfillClose with gain", async function () {
      ...
      const closeEvent = receipt!.logs.find(
        (l) =>
          c.futures.interface.parseLog(l as any)?.name ===
      ...
      const pnlHandle = await c.futures
        .connect(signers.alice)
        .getMyRealizedPnL();
    ...
    it("PnL accumulates across two profitable closes", async function () {
      ...
      const ev1 = rec1!.logs.find(
        (l) =>
          c.futures.interface.parseLog(l as any)?.name ===
          "PositionCloseRequested",
      );
      const reqId1 = c.futures.interface.parseLog(ev1 as any)!
        .args[2] as bigint;
      ...
      const ev2 = rec2!.logs.find(
        (l) =>
          c.futures.interface.parseLog(l as any)?.name ===
          "PositionCloseRequested",
      );
      const reqId2 = c.futures.interface.parseLog(ev2 as any)!
        .args[2] as bigint;
